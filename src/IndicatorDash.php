...
$sql = "SELECT id, question_text FROM indicator_questions";
$stmt = $pdo->query($sql);
$result = $stmt->fetchAll(PDO::FETCH_ASSOC);

$query = "SELECT * FROM indicator_responses WHERE user_id = ?";
    ...
    if ($userID !== null) {
        foreach ($_POST as $key => $value) {
            if (strpos($key, 'question_') !== false) {
                ...
                $insertSql = "INSERT INTO indicator_responses (user_id, question_id, response) VALUES (:user_id, :question_id, :response)";
                $insertStmt = $pdo->prepare($insertSql);
                $insertStmt->bindParam(':user_id', $userID, PDO::PARAM_INT);
                $insertStmt->bindParam(':question_id', $questionId, PDO::PARAM_INT);
                $insertStmt->bindParam(':response', $selectedChoice, PDO::PARAM_STR);
                if (!$insertStmt->execute()) {
                    // The query failed to execute
                    echo "Failed to insert data: " . implode(", ", $insertStmt->errorInfo());
                }
            }
        }
        echo "<pre>";
...
<body>
    ...
    <div class="wrapper d-flex align-items-stretch">
        ...
        <div id="content" class="p-4 p-md-5">
            ...
            if ($result) {
                ?>
                <h2>Indicator Questions</h2>
                <form method="post">
                    <table>
                        <thead>
                            <tr>
                                <th>Question Indicators</th>
                                <th>Strongly disgree</th>
                                <th>Disgree</th>
                                <th>Neutral</th>
                                <th>Agree</th>
                        ...
                        <tbody>
                            ...
                            foreach ($result as $row) {
                                ...
                                $choices = array("Strongly disagree", "Disagree", "Neutral", "Agree", "Strongly Agree", "Not Applicable");

                                echo "<tr>";
                                echo "<td>$questionText</td>";

                                foreach ($choices as $choice) {
                                    echo "<td><input type='radio' required name='question_$questionId' value='$choice'></td>";
                                }
                ...
                echo "No indicators found in the database.";
            }
            ?>
